...
> **Nota:** Quer entender mais sobre roteamento? Acesse a página ["por que um framework?"](/learn/why-frameworks) para uma explicação mais detalhada.
...
> **Nota:** Por padrão, quando um controlador é chamado dentro do framework, a classe `flight\Engine` é sempre injetada, a menos que você especifique por meio de um [contêiner de injeção de dependência](/learn/dependency-injection-container)
...
- **Requisições HEAD** são tratadas exatamente como requisições `GET`, mas o Flight remove automaticamente o corpo da resposta antes de enviá-la ao cliente.
...
> **Nota:** A correspondência de grupos de regex `()` com parâmetros posicionais não é suportada. Ex: `:'\(`
...
E se você acessar a seguinte URL: `/bob/123`, a saída seria `hello, 123 (bob)!`. _Por favor, tenha cuidado_ ao configurar suas rotas e suas funções de retorno!
...
Por padrão, se uma URL não puder ser encontrada, o Flight enviará uma resposta `HTTP 404 Not Found` muito simples e básica. Se você quiser ter uma resposta 404 mais personalizada, pode [mapear](/learn/extending) seu próprio método `notFound`:
...
Se você quiser ter uma resposta 405 mais personalizada, pode [mapear](/learn/extending) seu próprio método `methodNotFound`:
...
2. Você pode pedir para que o objeto de rota seja passado para seu retorno de chamada passando `true` como o terceiro parâmetro no método de rota. O objeto de rota será sempre o último parâmetro passado para sua função de retorno.
...
> **Nota:** A propriedade `executedRoute` só será definida depois que uma rota for executada. Se você tentar acessá-la antes de uma rota ser executada, ela será `NULL`. Você também pode usar executedRoute em [middleware](/learn/middleware) também!
...
> **Nota:** Este é o método preferido para definir rotas e grupos com o objeto `$router`.
...
Veja mais detalhes na página de [middleware de grupo](/learn/middleware#grouping-middleware).
...
## Veja Também
- [Middleware](/learn/middleware) - Usando middleware com rotas para autenticação, registro de logs, etc.
- [Injeção de Dependência](/learn/dependency-injection-container) - Simplificando a criação e o gerenciamento de objetos em rotas.
...
- [php.net: preg_match](https://www.php.net/manual/en/function.preg-match.php) - Função PHP para correspondência de expressões regulares.
...
- A propriedade executedRoute só é definida depois que uma rota é executada; ela é NULL antes da execução.
- Streaming exige que a funcionalidade legada de buffer de saída do Flight esteja desativada (`flight.v2.output_buffering = false`).
...
A razão para isso é um mecanismo especial embutido no roteador que trata a saída de retorno como um sinal para "ir para a próxima rota". Você pode ver o comportamento documentado na seção [Roteamento](/learn/routing#passing).
