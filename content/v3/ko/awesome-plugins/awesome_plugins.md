...
이메일 전송은 대부분의 웹 애플리케이션의 핵심 요구입니다. 환영 메시지, 비밀번호 재설정, 알림 등. 이 라이브러리들은 전달성을 안정적으로 유지하면서 전송을 쉽게 만들어 줍니다.

- [ryanstubbs/flightmail](/awesome-plugins/flightmail) - FlightMail은 Symfony Mailer를 유창하고 Flight에 친숙한 API로 래핑합니다. 간단한 DSN 문자열로 SMTP 또는 주요 제공자를 통해 보내고, 메시지마다 다른 제공자로 라우팅하며, Twig 또는 Latte 템플릿으로 본문을 렌더링합니다. 이는 Flight의 비공식 플러그인이며 Flight 팀이 유지보수하지 않습니다.
